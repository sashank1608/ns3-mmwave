...
map<uint64_t, uint16_t> imsi_cellid;
...
map<uint16_t, set<uint32_t>> ue_list;
...
std::string tracefilename1 = "/base_station_Data.csv";
std::string tracefilename2 = "/ue_base_station_energy_Data.csv";
...
trackCellId (uint32_t node_id, uint16_t old_cellid, uint16_t new_cellid)
{
  ...
  // if(old_cellid!=0)
  // {
  //     //uint16_t old_cellid = imsi_cellid[imsi];
      
  //     //imsi_list[old_cellid].erase(imsi);
  //     cout<<"Trace_3 * "<<Simulator::Now().GetSeconds()<<" "<<old_cellid<<" "<<connected_ue_energy[make_pair(old_cellid,node_id)]<<"\n";
  //     ue_list[old_cellid].erase(node_id);
  //     //[make_pair(old_cellid,nodeid)] = 0;
  //     //no_of_active_ue[old_cellid]-=active[node_id];
  // }
  // //imsi_cellid[imsi] = cellid;
  // //imsi_list[cellid].insert(imsi);
  // ue_list[new_cellid].insert(node_id);
  // //[make_pair(cellid,ueimsi_nodeid[imsi])] = 0;
  // //no_of_active_ue[cellid]+=active[node_id];
  // cout<<"Total ue connected to according to trackcellid "<<new_cellid<<" "<<ue_list[new_cellid].size()<<"\n";
...
{
  uint16_t cellid = ue_cellid_usinghandover[node_id];
  ue_energy_map[node_id] = totalnewEnergyConsumption;
  connected_ue_energy[make_pair(cellid,node_id)] = totalnewEnergyConsumption;
}

void
EnbEnergyConsumptionUpdate (uint16_t cell_id, double totaloldEnergyConsumption,
                         double totalnewEnergyConsumption)
{
  enb_energy_map[cell_id] = totalnewEnergyConsumption;
}

//assume initially all base stations sleep
void MakeBaseStationSleep (Ptr<MmWaveSpectrumPhy> endlphy, Ptr<MmWaveSpectrumPhy> enulphy, bool val,uint16_t cellid)
{
  ...
  // }
  if(val == 1) cout<<"Base Station going to sleep "<<cellid<<"\n";
  else cout<<"Base Station awakened "<<cellid<<"\n";
...
void make_base_station_sleep(uint16_t cellid,bool flag)
{
    Ptr<Node> nn =  cellid_node[cellid];
    Ptr<MmWaveEnbPhy> enbPhy =nn->GetDevice(0)->GetObject<MmWaveEnbNetDevice> ()->GetPhy ();
    Ptr<MmWaveEnbNetDevice> mmdev = DynamicCast<MmWaveEnbNetDevice> (nn->GetDevice(0));
    uint16_t cell_id = mmdev->GetCellId();
    if(cellid!=cell_id) cout<<"ERROR Stop simulation \n";
    else cout<<"ALL correct \n";
    Ptr<MmWaveSpectrumPhy> enbdl= enbPhy->GetDlSpectrumPhy ();
    Ptr<MmWaveSpectrumPhy> enbul= enbPhy->GetUlSpectrumPhy ();
    Simulator::Schedule(Simulator::Now(), &MakeBaseStationSleep, enbdl, enbul, flag,cell_id);
...
NotifyConnectionEstablishedUe (std::string context, uint64_t imsi, uint16_t cellid, uint16_t rnti)
{
  ...
  // if(imsi_cellid.find(imsi)!=imsi_cellid.end())
  // {
  //     uint16_t old_cellid = imsi_cellid[imsi];
      
  //     imsi_list[old_cellid].erase(imsi);
  //     cout<<"* "<<Simulator::Now().GetSeconds()<<" "<<cellid<<" "<<connected_ue_energy[make_pair(old_cellid,nodeid)]<<"\n";
  //     //[make_pair(old_cellid,nodeid)] = 0;
...
{
  uint32_t nodeid = ueimsi_nodeid[imsi];
  std::cout << Simulator::Now ().GetSeconds () << " " << context << " UE IMSI " << nodeid
...
NotifyHandoverEndOkUe (std::string context, uint64_t imsi, uint16_t cellid, uint16_t rnti)
{
  ...
  std::cout << Simulator::Now ().GetSeconds () << " " << context << " UE nodeid " << nodeid
...
NotifyConnectionEstablishedEnb (std::string context, uint64_t imsi, uint16_t cellid, uint16_t rnti)
{
   ...
   if(ue_cellid_usinghandover.find(nodeid)!=ue_cellid_usinghandover.end())
   {
  ...
  //     imsi_list[old_cellid].erase(imsi);
  //     cout<<"Trace_1 * "<<Simulator::Now().GetSeconds()<<" "<<cellid<<" "<<connected_ue_energy[make_pair(old_cellid,nodeid)]<<"\n";
  //     //[make_pair(old_cellid,nodeid)] = 0;
  //     no_of_active_ue[old_cellid]-=active[nodeid];
  // }
  // imsi_cellid[imsi] = cellid;
  ...
  std::cout << Simulator::Now ().GetSeconds () << " " << context << " eNB CellId " << cellid
            << ": successful connection of UE with IMSI " << nodeid << " RNTI " << rnti << std::endl;
                        ...
                        uint16_t targetCellId)
{
   uint32_t nodeid = ueimsi_nodeid[imsi];
  std::cout << Simulator::Now ().GetSeconds () << " " << context << " eNB CellId " << cellid
            << ": start handover of UE with IMSI " << nodeid << " RNTI " << rnti << " to CellId "
...
{
    uint32_t nodeid = ueimsi_nodeid[imsi];
  //ue_cellid_usinghandover[nodeid]; 
  ...
  if(ue_cellid_usinghandover.find(nodeid)!=ue_cellid_usinghandover.end())
  {
      ...
      if(ue_list[old_cellid].empty())
      {
          make_base_station_sleep(old_cellid,1);
      }
      //[make_pair(old_cellid,nodeid)] = 0;
      no_of_active_ue[old_cellid]-= active[nodeid];
  }
  ...
  //imsi_list[cellid].insert(imsi);
  ue_list[cellid].insert(nodeid);
  if(ue_list[cellid].size()==1)
  {
      make_base_station_sleep(cellid,0);
  }
  //[make_pair(cellid,ueimsi_nodeid[imsi])] = 0;
  no_of_active_ue[cellid]+= active[nodeid];
  //cout<<"Total ue connected to according to trackcellid "<<cellid<<" "<<ue_list[cellid].size()<<"\n";
  //int imsi_count = imsi_list[cellid].size ();
  ue_cellid_usinghandover[nodeid] = cellid;
  std::cout << "Total number of UE " << ue_list[cellid].size() << " Connected to Cell " << cellid << std::endl;

  std::cout << Simulator::Now ().GetSeconds () << " " << context << " eNB CellId " << cellid
            << ": completed handover of UE with IMSI " << nodeid << " RNTI " << rnti << std::endl;
}
  ...
  if (Simulator::Now ().GetSeconds () * 10 > 40)
    {
      ...
      uint16_t connected_cellid = ue_cellid_usinghandover[nodeid];
...
void base_station_data(uint16_t cellid)
{
   double currenttime = Simulator::Now ().GetSeconds ();
    std::ofstream outFile;
...
void stop_ue_application(Ptr<Application> app, uint32_t nodeid)
{
   cout<<"Stopped ue app on "<<nodeid<<"\n";
  active[nodeid] = 0;
  int cellid = ue_cellid_usinghandover[nodeid];
  no_of_active_ue[cellid]--;
  app->SetStopTime(Simulator::Now());
}
void start_ue_application(Ptr<Application> app, uint32_t nodeid)
{
  cout<<"Started ue app on "<<nodeid<<"\n";
...
//   mmwaveHelper->SetChannelConditionModelType ("ns3::BuildingsChannelConditionModel");
 ...
 for (uint32_t i = 0; i < mmWaveEnbNodes.GetN(); i++)
  ...
  for(int i = 0; i < gNbNum; i++)
   {
    ...
    // Positions
  // Vector mmw1Position = Vector (50, 70, gNbHeight);
  // Vector mmw2Position = Vector (300, 70, gNbHeight);
  // Vector mmw3Position = Vector (550, 70, gNbHeight);
  ...
  //apPositionAlloc->Add (mmw1Position); // LTE BS, out of area where buildings are deployed
  // apPositionAlloc->Add (mmw1Position);
  // apPositionAlloc->Add (mmw2Position);
  // apPositionAlloc->Add (mmw3Position);
...
//   ueNodes.Get (0)->GetObject<ConstantVelocityMobilityModel> ()->SetVelocity (Vector (0, 0, 0));
 ...
 for (uint32_t i = 0; i < ueNodes.GetN (); i++)
  ...
  // basicSourceHelper_ue.Set ("BasicEnergySourceInitialEnergyJ", DoubleValue (10));
  // basicSourceHelper_ue.Set ("BasicEnergySupplyVoltageV", DoubleValue (5.0));
  // basicSourceHelper_ue.Set ("BasicEnergySourceInitialEnergyJ", DoubleValue (1000000000.0));
  // EnergySourceContainer sources_ue = basicSourceHelper_ue.Install (ueNodes);
  // MmWaveRadioEnergyModelEnbHelper nrEnergyHelper_ue;
  // DeviceEnergyModelContainer deviceEnergyModel_ue = nrEnergyHelper_ue.Install (mcUeDevs, sources_ue);
  // for (uint32_t u = 0; u < ueNodes.GetN (); ++u)
  // {
   ...
   if (tcp)
    {
      ...
      for (uint32_t u = 0; u < sinkApps.GetN (); ++u)
      {
         double val = rand()% ((int)appTime)+ 0.1;
         Simulator::Schedule (Seconds (val), &start_ue_application, sinkApps.Get (u),ueNodes.Get (u)->GetId ());
      }
      //sinkApps.Get(0);
      //sinkApps.Start (Seconds (1));
      //sinkApps.Stop (Seconds (appTime));
     ...
     // Ptr<OutputStreamWrapper> stream1 = asciiTraceHelper.CreateFileStream ("mmWave-tcp-window-newreno.txt");
      ...
      app->SetStopTime (Seconds (appTime));
    }
  else
    {
      // Install and start applications on UEs and remote host
      ...
      for (uint32_t u = 0; u < sinkApps.GetN (); ++u)
      {
          double val = min(u+1.1,appTime-1);//rand()% ((int)appTime);
         Simulator::Schedule (Seconds (val), &start_ue_application, sinkApps.Get (u),ueNodes.Get (u)->GetId ());
      }
  ...
  // Start applications
  std::ofstream tracefile,tr;
  tracefile.open (outputDir + tracefilename1, std::ios::out | std::ios::trunc);
...
for (uint32_t u = 0; u < mmWaveEnbDevs.GetN (); ++u)
    {
      Ptr<MmWaveEnbNetDevice> mmdev = DynamicCast<MmWaveEnbNetDevice> (mmWaveEnbNodes.Get(u)->GetDevice(0));
      Simulator::Schedule (Seconds (1), &base_station_data, mmdev->GetCellId());
    }

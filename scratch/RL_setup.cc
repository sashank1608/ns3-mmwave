...
//   ueNodes.Get (0)->GetObject<ConstantVelocityMobilityModel> ()->SetVelocity (Vector (0, 0, 0));
 ...
 for (uint32_t i = 0; i < ueNodes.GetN (); i++)
  ...
  // make base station to go to sleep
    Ptr<MmWaveEnbPhy> enbPhy = mmWaveEnbNodes.Get(0)->GetDevice(0)->GetObject<MmWaveEnbNetDevice> ()->GetPhy ();
    Ptr<MmWaveEnbNetDevice> mmdev = DynamicCast<MmWaveEnbNetDevice> (mmWaveEnbNodes.Get(0)->GetDevice(0));
    Ptr<MmWaveSpectrumPhy> enbdl= enbPhy->GetDlSpectrumPhy ();
    Ptr<MmWaveSpectrumPhy> enbul= enbPhy->GetUlSpectrumPhy ();
    Simulator::Schedule(Seconds(4), &MakeBaseStationSleep, enbdl, enbul, false,mmdev->GetCellId());
    Simulator::Schedule(Seconds(8), &MakeBaseStationSleep, enbdl, enbul, true,mmdev->GetCellId());
  ...
  for (uint32_t u = 0; u < mmWaveEnbNodes.GetN (); ++u)
  {
    ...
    deviceEModel.Get(0)->TraceConnectWithoutContext ("TotalEnergyConsumption", MakeBoundCallback (&EnbEnergyConsumptionUpdate,mmdev->GetCellId()));

...
using namespace std;
double max_idle = -1,min_idle = 1e7;
double max_ctrl = -1,min_ctrl = 1e7;
double max_data = -1,min_data = 1e7;
double max_tx = -1,min_tx = 1e7;
double idle_t=0;
double tx_t=0;
double ctrl_t=0;
double data_t=0;
...
string time_file="energytimewithnoapplication.csv";
void 
update_idle(uint32_t nodeid , double old_val, double new_val)
{
    
    
    max_idle = max(max_idle,new_val);
    min_idle = min(min_idle,new_val);
    idle_t+=new_val;
    std::ofstream outFile;
    outFile.open (time_file,std::ios_base::out | std::ios_base::app);
    outFile << Simulator::Now ().GetSeconds () <<", idle "<< "," << new_val << "," <<idle_t<<"\n";

}
void 
update_ctrl(uint32_t nodeid , double old_val, double new_val)
{
    max_ctrl = max(max_ctrl,new_val);
    min_ctrl = min(min_ctrl,new_val);
    ctrl_t+=new_val;
    std::ofstream outFile;
    outFile.open (time_file,std::ios_base::out | std::ios_base::app);
    outFile << Simulator::Now ().GetSeconds () <<", ctrl "<< "," << new_val << "," <<ctrl_t<<"\n";

}
void 
update_data(uint32_t nodeid , double old_val, double new_val)
{
    max_data = max(max_data,new_val);
    min_data = min(min_data,new_val);
    data_t+=new_val;
    std::ofstream outFile;
    outFile.open (time_file,std::ios_base::out | std::ios_base::app);
    outFile << Simulator::Now ().GetSeconds () <<", data "<< "," << new_val << "," <<data_t<<"\n";

}
void 
update_tx(uint32_t nodeid , double old_val, double new_val)
{
    max_tx = max(max_tx,new_val);
    min_tx = min(min_tx,new_val);
    tx_t+=new_val;
    std::ofstream outFile;
    outFile.open (time_file,std::ios_base::out | std::ios_base::app);
    outFile << Simulator::Now ().GetSeconds () <<", tx "<< "," << new_val << "," <<tx_t<<"\n";

}

string rrcstatefile="rrcstatechangewithapplication.csv";
void HandleRRCStateChange(std::string context,uint64_t m_imsi,uint16_t m_cellId,uint16_t m_rnti, ns3::LteUeRrc::State oldState, ns3::LteUeRrc::State newState)
{
    ...
    std::ofstream outFile;
     ...
     outFile << Simulator::Now ().GetSeconds () << "," << rrcStates[oldState] << "," <<rrcStates[newState]<<std::endl;
     cout<<Simulator::Now ().GetSeconds () << " , " << rrcStates[oldState] << " , " <<rrcStates[newState]<<std::endl;
...
main (int argc, char *argv[])
{
  ...
  outFile1.open (time_file,std::ios_base::out | std::ios_base::trunc);
  outFile1 << "Time,state,timespent,totaltime" << std::endl;
  ...
  outFile.open (rrcstatefile,std::ios_base::out | std::ios_base::trunc);
  outFile << "Time,oldstate,newstate" << std::endl;


  Config::Connect ("/NodeList/*/DeviceList/*/LteUeRrc/StateTransition",
                   MakeCallback (&HandleRRCStateChange));
...
//   clientApps.Stop (Seconds (simTime - 1));
  ...
  cout<<"min idle: "<<min_idle<<" max idle: "<<max_idle <<" total idle time: "<<idle_t<<"\n";
  cout<<"min data: "<<min_data<<" max data: "<<max_data<<" total data time: "<<data_t<<"\n";
  cout<<"min tx: "<<min_tx<<" max tx: "<<max_tx<<" total tx time: "<<tx_t<<"\n";
  cout<<"min ctrl: "<<min_ctrl<<" max ctrl: "<<max_ctrl<<"total ctrl time: "<<ctrl_t<<endl;
  Simulator::Destroy ();
  return 0;
}

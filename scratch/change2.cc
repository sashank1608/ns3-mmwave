...
string rrcstatefileEnb1="rrcstatechange_enb1.csv";
string rrcstatefileEnb2="rrcstatechange_enb2.csv";
void HandleRRCStateChangeEnb(std::string context,uint64_t m_imsi,uint16_t m_cellId,uint16_t m_rnti, ns3::LteUeRrc::State oldState, ns3::LteUeRrc::State newState)
{
   const std::string rrcStates[] =
    {   
        "IDLE_START",
  ...
  {
       std::ofstream outFile;
      outFile.open (rrcstatefileEnb1,std::ios_base::out | std::ios_base::app);
      outFile << Simulator::Now ().GetSeconds () << "," << rrcStates[oldState] << "," <<rrcStates[newState]<<std::endl;
     cout<<Simulator::Now ().GetSeconds () <<" "<<nval<<" , " << rrcStates[oldState] << " , " <<rrcStates[newState]<<std::endl;
  }
  else
  {
     std::ofstream outFile;
      outFile.open (rrcstatefileEnb2,std::ios_base::out | std::ios_base::app);
      outFile << Simulator::Now ().GetSeconds () << "," << rrcStates[oldState] << "," <<rrcStates[newState]<<std::endl;
     cout<<Simulator::Now ().GetSeconds () <<" "<<nval<<" , " << rrcStates[oldState] << " , " <<rrcStates[newState]<<std::endl;
...
Config::Connect ("/NodeList/*/DeviceList/*/LteEnbRrc/StateTransition",
                   MakeCallback (&HandleRRCStateChangeEnb));
  ...
  cout<<"min idle"<<min_idle<<" max idle "<<max_idle<<endl;
  cout<<"min data "<<min_data<<" max data "<<max_data<<endl;
  cout<<"min tx "<<min_tx<<" max tx "<<max_tx<<endl;
  cout<<"min ctrl "<<min_ctrl<<" max ctrl "<<max_ctrl<<endl;
  Simulator::Destroy ();
  return 0;
}
